...
export default class upIdCard extends Component {
    ...
    componentDidMount () {
        ...
        document.addEventListener('dragstar', function (e) {
            e.preventDefault();
        });
        this.refs.row_box.addEventListener('drap', (e) => {
    ...
    render () {
        return (
            <div>
                <Row>
                    <Col span={4}><h3>{this.props.c}：</h3></Col>
                    <Col span={4}>
                        <Button
                            onClick={()=>{
                                //创建一个事件
                    ...
                    <input
                        ...
                        onChange={()=>{
                            this.setState({
                                files: [...this.state.files, ...[...this.refs.pic.files].map(f=>({
                                    'id': Math.random(),
                                    'file': f
                                }))]
                            });
                ...
                <div className="row_box" ref="row_box">
                    {
                        this.state.files.map((item, index)=><OnePic
                            key={item.id}
                            info={item}
                            another={{'del': this.del.bind(this)}}
                        />)
                    }
                </div>
            </div>
